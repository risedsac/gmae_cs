...
func build_viewmodel():
	...
	for index in 4:
		var model=W.make(index);weapon_anchor.add_child(model)
		model.scale=Vector3.ONE*.78;models.append(model)
	...
	flash=OmniLight3D.new();flash.light_color=Color(1,.65,.18);flash.light_energy=0;flash.omni_range=2;weapon_anchor.add_child(flash)
	flash.position=Vector3(0,0,-1.3)
...
func reset_at(pos: Vector3):
	global_position=pos+Vector3.UP*.08;velocity=Vector3.ZERO;rotation.y=0;pitch=0;scoped=false;rescope_after_shot=false
	if hp<=0:armor=0
...
func _unhandled_input(event):
	...
	if event is InputEventMouseMotion and Input.mouse_mode==Input.MOUSE_MODE_CAPTURED:
		var modifier=.32 if scoped else 1.
		rotate_y(-event.relative.x*sensitivity*modifier);pitch=clampf(pitch-event.relative.y*sensitivity*modifier,-1.48,1.48)
		sway+=event.relative*.0006
	...
	if event.is_action_pressed("scope") and weapon==3:scoped=not scoped;rescope_after_shot=false
	if event.is_action_pressed("he"):game.throw_grenade(self,"he",-camera.global_basis.z,camera.global_position)
	if event.is_action_pressed("smoke_grenade"):game.throw_grenade(self,"smoke",-camera.global_basis.z,camera.global_position)

func equip_primary(index: int):
	for i in [0,2,3]:owned[i]=i==index
	primary=index;ammo[index]=capacities[index];reserve[index]=W.DATA[index].reserve
	switch_weapon(index)

func switch_weapon(index: int):
	if not owned[index] or index==weapon:return
	if is_instance_valid(reload_audio):reload_audio.stop()
	reload_left=0;reload_queued=false;shot_queued=false;weapon=index;draw_left=.28;shot_cooldown=.28;scoped=false;rescope_after_shot=false
	for i in 4:models[i].visible=i==weapon

func request_reload():
	if reload_left>0 or ammo[weapon]==capacities[weapon] or reserve[weapon]<=0:return
	scoped=false;rescope_after_shot=false
	reload_duration=W.DATA[weapon].reload;reload_left=reload_duration
...
func _physics_process(dt):
	...
	if hp<=0:
		...
		if not allies.is_empty():
			...
			camera.global_transform=Transform3D(ally.global_basis,ally.global_position+Vector3.UP*1.62)
		return
	protection=maxf(0,protection-dt);shot_cooldown=maxf(0,shot_cooldown-dt)
	...
	capsule.shape.height=1.2 if crouched else 1.8;capsule.position.y=capsule.shape.height/2
	...
	var speed=2.25 if Input.is_action_pressed("walk") else 5.1
	if crouched:speed=1.65
	if weapon==3:speed*=.8
	if scoped:speed*=.65
	var accel=30. if is_on_floor() else 7.
	velocity.x=move_toward(velocity.x,direction.x*speed,accel*dt);velocity.z=move_toward(velocity.z,direction.z*speed,accel*dt)
	...
	if flat_speed>.5 and is_on_floor():
		...
		if step_clock<=0:
			step_clock=.44 if speed>3 else .6
			game.sound.local("step"+str(randi()%6),-22 if speed>3 else -32)
			game.noise(global_position,11. if speed>3 else 2.5,self)
	# Shot input is consumed before reload input, including quick click + R in one frame.
	camera.rotation.x=pitch
	game.interact(self,Input.is_action_pressed("interact") and not game.hud.shop.visible,dt)
	if shot_queued or (weapon in [0,2] and Input.is_action_pressed("fire")):fire()
	shot_queued=false
	if reload_queued:request_reload();reload_queued=false
	update_view(dt,flat_speed)

func fire():
	if reload_left>0 or shot_cooldown>0 or hp<=0 or game.phase not in ["live","planted"] or game.hud.shop.visible or game.interaction_actor==self:return
	...
	ray_dir=(ray_dir+camera.global_basis.x*randfn(0,spread)+camera.global_basis.y*randfn(0,spread)).normalized()
	var origin=camera.global_position
	var hit=game.ray(origin,origin+ray_dir*100,1|4,[get_rid()])
	var target=origin+ray_dir*80 if hit.is_empty() else hit.position
	# Camera aims, but a second ray from the world muzzle prevents shooting through cover.
	var muzzle=camera.global_position+camera.global_basis*Vector3(.20,-.22,-.5)
	var near_hit=game.ray(origin,muzzle,1,[get_rid()])
	...
	if not hit.is_empty():
		if hit.collider.has_method("take_damage"):
			var head=target.y-hit.collider.global_position.y>1.43
			var damage=W.DATA[weapon].head if head else W.DATA[weapon].damage
			if hit.collider.team!=team:
				hit.collider.take_damage(damage,head,self);game.hit_marker=.16
		else:game.impact(target,hit.normal)
	game.tracer(muzzle,target,Color(1,.74,.30));game.noise(global_position,42.,self)
	game.sound.local(W.DATA[weapon].sound,-5 if weapon!=3 else -4)
	...
	flash_time=.045
	kick_velocity+=Vector3(.55 if weapon!=3 else 1.0,randf_range(-.10,.10),1.8 if weapon!=3 else 3.1)

func update_view(dt: float,speed: float):
	recoil=move_toward(recoil,0,dt*2.5);sway=sway.lerp(Vector2.ZERO,minf(1,dt*10))
	sway=sway.limit_length(.035);camera.rotation.x=pitch
	camera.fov=25 if scoped and weapon==3 else 80
	weapon_anchor.visible=hp>0 and not (scoped and weapon==3)
	var base=Vector3(.24,-.24,-.47) if weapon!=1 else Vector3(.20,-.16,-.45)
	var pose=Vector3(.025,.07,-.015)
	breathing+=dt;motion_blend=lerpf(motion_blend,minf(speed/3.,1.),1-exp(-dt*9))
	# Critically damped return separates the fast shot impulse from the slower aiming recoil.
	kick_velocity+=(-kick_position*190.-kick_velocity*23.)*dt
	kick_position+=kick_velocity*dt
	base+=Vector3(0,sin(breathing*1.7)*.0025,kick_position.z)
	pose+=Vector3(kick_position.x,kick_position.y,0)
	draw_left=maxf(0,draw_left-dt)
	var draw=draw_left/.28
	base.y-=draw*draw*.22;pose.x-=draw*.20
	base+=Vector3(sin(bob*.5)*.006,absf(cos(bob))*.009,0)*motion_blend
	base+=Vector3(-sway.x,sway.y,recoil*.035);pose+=Vector3(recoil*.025,-sway.x,-sway.x*.5)
	for i in 4:
	...
	if reload_left>0:
		var t=1-reload_left/reload_duration
		var dip=smoothstep(0.,.15,t)*(1-smoothstep(.82,1.,t))
		base+=Vector3(-.10,.045,.04)*dip;pose.z-=dip*.43;pose.y-=dip*.12;pose.x+=dip*.04
		var reach=smoothstep(.10,.27,t)*(1-smoothstep(.76,.96,t))
		var extract=smoothstep(.27,.44,t)*(1-smoothstep(.52,.74,t))
		var grip_shift=Vector3(.015,-.15 if weapon!=1 else -.085,[.34,.03,.27,.34][weapon])
		var mag_shift=Vector3(-.07,-.40,.10)*extract
		if magazines[weapon]:
			magazines[weapon].position=mag_shift
			magazines[weapon].visible=not (t>.44 and t<.52)
	...
	if weapon==3 and shot_cooldown>0 and reload_left<=0:
		var cycle=1-shot_cooldown/W.DATA[3].interval
		var pull=sin(clampf((cycle-.14)/.62,0,1)*PI)
	...
	flash_time=maxf(0,flash_time-dt);flash.light_energy=2.8 if flash_time>0 else 0.
...
func take_damage(amount: int,head=false,source=null):
	...
	var absorbed=mini(armor,roundi(amount*.35));armor-=absorbed;hp=maxi(0,hp-amount+absorbed)
	game.hurt_overlay=.55
	if hp==0:
		if is_instance_valid(reload_audio):reload_audio.stop()
		reload_left=0;scoped=false;camera.fov=80;weapon_anchor.visible=false;velocity=Vector3.ZERO;collision_layer=0;collision_mask=0
