...
# Realistic replacement art. AK/M4 are Godot-ready authored PBR GLBs; the
# pistol and sniper are resolved from the Stein Games Classic Weapons Pack
# manifest generated by tools/install-realistic-assets.sh. Missing assets fall
# back to the original Dustline models so a clean clone still runs.
...
# Steel Tide's authored first-person presentation uses these scales. Stein's
# FBX files are imported by Godot with their authored unit scale.
...
static func realistic_path(index: int) -> String:
	...
	var fixed=FIXED_PATHS[index]
	if not fixed.is_empty() and ResourceLoader.exists(fixed):return fixed
	if not FileAccess.file_exists(MANIFEST):return ""
	var cfg=ConfigFile.new()
	if cfg.load(MANIFEST)!=OK:return ""
...
static func load_realistic_weapon(index: int) -> Node3D:
	...
	var scene=load(path)
	if not scene or not scene.has_method("instantiate"):return null
	...
	# Stein Games authors for an Unreal-style +X-forward pipeline. Rotate its
	# FBX long axis into Dustline's -Z-forward viewmodel frame.
	if path.contains("stein_classic_weapons"):visual.rotation.y=PI/2
	root.add_child(visual)
	# Reuse the current CC0 first-person arms until the hand/animation pass is
	# replaced separately; this keeps reload and weapon sway behavior intact.
	...
	donor.free()
	# Do not let the legacy animation code zero an authored magazine transform.
	# The upgraded gun remains visually intact; these animation anchors are
	# intentionally separate until a full source-rig reload pass is added.
