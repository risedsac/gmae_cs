...
func load_realistic_profile(sound: String):
	...
	var roles={
		"near":profile+"_player_near.wav",
		"world":profile+"_world.wav",
		"distant":profile+"_enemy_distant.wav"
	}
	var loaded={}
	for role in roles:
...
func play_at(sound: String,pos: Vector3,volume=-6.):
	...
	if sound in GUNS and is_instance_valid(game.player):
		...
		distance=ear.distance_to(pos)
		blocked=not game.ray(ear,pos,1).is_empty()
	p.stop()
	p.global_position=pos
	p.pitch_scale=randf_range(.992,1.008) if sound in GUNS else 1.
	...
	if sound in GUNS and realistic_streams.has(sound):
		...
		p.stream=realistic_streams[sound][role]
		p.volume_db=volume-(7. if blocked else 0.)
	else:
		var suffix=""
...
func local(sound: String,volume=-6.) -> AudioStreamPlayer:
	for p in ui_players:
		if not p.playing:
			...
			p.volume_db=volume
			p.pitch_scale=randf_range(.994,1.006) if sound in GUNS else 1.
			p.play();return p
	return null
