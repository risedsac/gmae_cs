...
def read_wav(path: Path) -> tuple[int, array]:
    with wave.open(str(path), "rb") as r:
        ...
        rate=r.getframerate();channels=r.getnchannels();raw=r.readframes(r.getnframes())
    values=array("h");values.frombytes(raw)
    if sys.byteorder!="little":values.byteswap()
    if channels==2:
        values=array("h",((int(values[i])+int(values[i+1]))//2 for i in range(0,len(values)-1,2)))
    elif channels!=1:
...
def peaks(samples: array, rate: int, count=3) -> list[int]:
    hop=max(1,int(rate*.004));window=max(hop,int(rate*.018));stride=max(1,hop//5)
    energy=[]
    for start in range(0,max(1,len(samples)-window),hop):
        peak=max((abs(int(samples[i])) for i in range(start,min(len(samples),start+window),stride)),default=0)
        energy.append((peak,start))
    chosen=[]
    for _,frame in sorted(energy,reverse=True):
        if all(abs(frame-old)>int(rate*.16) for old in chosen):
            chosen.append(frame)
            if len(chosen)>=count:break
    chosen.sort()
    while len(chosen)<count:chosen.append(chosen[-1] if chosen else 0)
    return chosen


def event_slice(samples: array, rate: int, center: int, pre=.045, post=.24) -> array:
    start=max(0,center-int(rate*pre));end=min(len(samples),center+int(rate*post))
    result=array("h",samples[start:end]);fade=min(len(result),int(rate*.045))
    for i in range(fade):
        idx=len(result)-fade+i;gain=1-i/max(1,fade-1);result[idx]=round(int(result[idx])*gain)
    return result


def normalize(samples: array,target=.62) -> array:
    p=max((abs(int(v)) for v in samples),default=0)
    if p<=0:return samples
    g=min(5.,target*32767./p)
    return array("h",(max(-32768,min(32767,round(int(v)*g))) for v in samples))
